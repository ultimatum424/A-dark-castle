...
void WorkGame(StructGame& game, RenderWindow& window)
{
	...
	if (game.stage_game == 0)
	{
		if ((game.key_event.key_escape))
			window.close();
		UpdateMenu(game.menu, game.key_event, game.stage_game, game.view.view_ñentre);
	...
	{
		ShopMode(game.inventory, game.key_event, game.view.view_ñentre, game.sound_effect, game.stage_game);
